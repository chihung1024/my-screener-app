...
def get_stock_data(ticker_symbol):
    ...
    try:
        ...
        # 在 API 環境中，我們將錯誤印在伺服器控制台
        # print(f"錯誤: 在獲取 {ticker_symbol} 的 Ticker 物件時失敗: {e}")
        return None
...
def calculate_metrics(data):
    ...
    try:
        ...
        # print(f"處理 {ticker_symbol} 指標計算時發生錯誤: {e}")
        return None
...
def screen_stocks():
    """
    這是一個 API 端點，接收 POST 請求來執行篩選。
    請求的 Body 應為 JSON 格式，包含 "tickers" 和 "weights"。
    """
    print("收到一個新的篩選請求...")
    try:
        # 1. 解析傳入的請求數據
        ...
        # 2. 執行篩選流程
        all_metrics = []
        print(f"準備處理 {len(tickers_to_process)} 支股票...")
        for ticker in tickers_to_process:
            ...
            time.sleep(1) # 關鍵延遲，避免被 Yahoo Finance 封鎖

        if not all_metrics:
            return jsonify({"error": "未能從 API 獲取任何有效的股票數據。"}), 500
            
        # 3. 數據處理與排名
        metrics_df = pd.DataFrame(all_metrics).replace([np.inf, -np.inf], np.nan)
        final_ranked_df = rank_stocks(metrics_df, factor_weights)

        # 4. 將最終結果轉換為 JSON 格式並返回
        # 將 NaN 轉換為 None (null in JSON)
        ...
        # 捕捉所有潛在的錯誤，並返回一個詳細的錯誤訊息
        print(f"伺服器內部錯誤: {e}")
        traceback.print_exc() # 在伺服器控制台印出完整的錯誤堆疊
        return jsonify({"error": "伺服器內部發生未知錯誤", "message": str(e)}), 500


# --- 啟動伺服器 ---

if __name__ == '__main__':
    # 讓伺服器在本地網路的所有 IP 上監聽，方便測試
    # 在 debug=True 模式下，每次儲存程式碼後伺服器會自動重啟
